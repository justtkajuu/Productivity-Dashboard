...
function openFeatures(){
    ...
    fullElemPageBackBtn.forEach(function(back){
        back.addEventListener('click',function(){
            gsap.to(fullElemPage[back.id], {
            opacity: 0,
            duration: 0.4
            });
            setTimeout(() => {
                fullElemPage[back.id].style.display = 'none';
                fullElemPage[back.id].style.opacity = 1;
            }, 400);
    })
...
hours.forEach(function(elem,idx){
    ...
    var savData =dayPlanData[idx] ||''
    wholeDaySum += `<div class="daily-planner-time">
                    <p>${elem}</p>
                    <input id= ${idx} type="text" placeholder="..." value = ${savData}>
                </div>`
})

dayPlanner.innerHTML = wholeDaySum

var dayPlannerinput = document.querySelectorAll('.daily-planner .daily-planner-time input')
...
async function weatherAPICall(){
    var apiKey = 'a46f8cc9a75c4c86a68150459253006'
    var city = 'Nagpur'
...
function timeDate(){
    ...
    header2Date.innerHTML = `${tarik} ${month}, ${year}`
